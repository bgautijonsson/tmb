...
TMB::compile("src/gev_multiple_stations_with_tcopula_precision_matrix.cpp")
dyn.load(dynlib("src/gev_multiple_stations_with_tcopula_precision_matrix"))
...
nu <- 0
df <- 4
to_pseudo_obs <- function(x) {
  apply(x, 2, function(col) {
    rank(col) / (length(col) + 1)
  })
}

U <- data$Y |> to_pseudo_obs()
S <- qt(U, df = df)

objective_student <- function(params) {
  -sum(dmatern_tcopula_eigen(S, n_x, n_y, params[1], params[2], nu, df))
}
...
rho <- res_student$solution

Q <- make_standardized_matern_eigen(n_x, n_y, rho[1], rho[2], nu)
...
data_list_copula <- list(
  ...
  prior_sigma = prior_sigma,
  df = 4,
  log_det_Q = 2 * (chol(Q) |> diag() |> log() |> sum())
)
...
obj_copula <- MakeADFun(
  ...
  DLL = "gev_multiple_stations_with_tcopula_precision_matrix",
  # silent = TRUE,
  hessian = TRUE,
  profile = c("phi", "tau", "psi"),
  inner.control = list(
